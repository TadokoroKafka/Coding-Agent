...
- 自行实现六个本地工具：列出文件、文本搜索、读取文件、写入文件、精确替换和执行命令。
- 将所有文件与命令操作限制在指定工作区内，防止绝对路径、父目录和符号链接越界。
- 默认使用 `ask` 模式；文件修改前展示本地统一差异，并允许批准、拒绝或向模型反馈修改意见。
- 提供适用于受控演示的受限 `auto` 模式。
- 仅重试模型 API 请求，并实现工具调用防重复和确定性循环终止。
- 上下文裁剪时完整保留工具调用与结果，并在裁剪后插入不含早期文件正文的压缩执行状态快照。
- 普通模式在每次同步模型请求前显示步骤心跳；`--verbose` 模式额外显示脱敏的工具进度和确定性运行总结，不依赖模型自行回忆执行过程。
...
- `cli.py`：组装应用、读取任务、显示脱敏进度与运行总结，并将运行结果映射为退出码。
- `model_client.py`：适配 DeepSeek 响应，仅对可恢复的 API 错误进行重试。
- `agent.py`：控制工具调用循环、防重复执行、错误计数与终止条件。
- `context.py`：只按完整交互组裁剪历史，并维护搜索、读取、修改、命令、测试和失败状态快照。
...
| `--approval-mode ask` | 默认模式。读取自动执行；写入、替换和命令使用 `y/N/f` 确认。 |
| `--approval-mode auto` | 允许文件修改，命令仅允许 `python`、`pytest`、`git status` 和 `git diff`。 |
| `--max-steps N` | 模型请求次数上限，默认值为 `20`。 |
| `--verbose` | 额外显示经过脱敏的工具、结果状态，以及任务结束后的确定性运行总结。 |

普通模式会在每次同步模型请求前显示 `[步骤 N] 正在请求模型`，用于避免等待期间没有反馈；这不是按 token 输出的流式响应。

`auto` 模式是用于受控演示的信任模式，并不是完整的安全沙箱；启动时程序会再次显示警告。`python` 和 `pytest` 都可能执行工作区代码，请只在本人完全控制的隔离工作区中使用。普通使用保持默认 `ask`，并依靠工作区限制和副作用确认控制风险。

### 可审查的修改确认

在 `ask` 模式下，`write_file` 和 `replace_in_file` 执行前会由本地代码生成 unified diff。新文件从 `/dev/null` 开始显示，已有文件显示修改前后的行；预览最多 60 行或 4000 个字符，过长时按字符边界截断。预览只显示在当前终端，不写入 JSONL 日志，并使用与日志相同的敏感信息脱敏规则。

确认提示支持以下输入：

- `y` 或 `yes`：批准并执行一次本地工具；
- `n`、`no`、空输入或其他输入：拒绝本次调用；
- `f` 或 `feedback`：输入一条修改意见，拒绝当前调用，并将意见作为结构化工具结果回传模型，由模型决定下一步。

差异预览只用于辅助人工审查，不是事务隔离。获批后工具仍会重新校验路径、UTF-8 编码和精确匹配次数；本地工具不会因拒绝或失败而自动重放。`auto` 模式不显示差异预览。

## 本地工具

| 工具 | 行为 |
|---|---|
| `list_files` | 默认递归列出最多 500 个匹配的工作区文件，并忽略 Git、虚拟环境、缓存和运行日志。 |
| `search_text` | 在指定文件范围内进行字面量文本搜索，返回文件路径、行号和匹配行；非 UTF-8 文件会被安全跳过。 |
...
- 文件修改先进行只读预演；预演失败时不询问、不执行，并将结构化错误回传模型。
...
- 裁剪快照只保存已读取或修改的路径、近期命令和失败状态，不复制可能已经过期的早期文件内容；任务需要依赖早期代码细节时，模型应重新调用 `read_file` 或 `search_text`。
...
测试覆盖文件隔离、文本搜索、Unicode 处理、精确替换、差异预演、反馈式权限策略、安全命令、超时清理、API 重试、模型输出解析、工具调用防重复、循环保护、上下文裁剪、确定性运行总结、日志和使用假模型的命令行端到端场景。

Windows 子进程树清理测试应在普通本地 PowerShell 或 PyCharm 会话中运行，因为受限执行环境可能会阻止 `taskkill`。

默认 `pytest` 会排除 `live` 标记，不访问网络，也不会产生模型费用。需要显式验证真实 DeepSeek thinking tool calling 和 `reasoning_content` 回传时，可在当前 PowerShell 临时设置 key 后运行：
...
真实测试会调用 API，可能产生费用；未设置 `DEEPSEEK_API_KEY` 时会明确显示 `SKIPPED`。测试从 `DEEPSEEK_BASE_URL` 和 `DEEPSEEK_MODEL` 读取可选覆盖值，并强制开启 thinking，但不会将 key 写入仓库或测试日志。
