...
这是一个为南京大学软件工程专业推免考核开发的独立 Python 命令行编程智能体。项目通过模型厂商的原生工具调用接口与大语言模型交互，智能体循环、本地工具、上下文管理、权限确认和错误处理均在本仓库中自行实现。
...
AI 工具用于辅助设计审查、实现讨论、测试规划、文档整理和问题调试。项目作者对系统架构、源代码、验证结果、安全边界以及考核中说明的每一项设计决策负责。

## 许可说明

本项目暂未选择开源许可证。仓库公开仅用于考核与审阅；仓库可公开访问并不代表授予代码复用许可。
